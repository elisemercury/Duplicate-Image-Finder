...
class build:
    '''
    A class used to initialize and build difPy objects
    '''
    def __init__(self, *directory, recursive=True, in_folder=False, limit_extensions=True, px_size=50, show_progress=True, logs=True):
        ...
        init = _initialize(self.__directory, self.__recursive, self.__in_folder, self.__limit_extensions, self.__px_size, self.__show_progress)
        self._tensor_dictionary, self._filename_dictionary, self._id_to_group_dictionary, self._group_to_id_dictionary, self._invalid_files, self._stats = init._main()
# check lower quality csv TODO
class search:
    ...
    def __init__(self, difpy_obj, similarity='duplicates', show_progress=True, logs=True):
        ...
        if logs:
            ...
            self.stats =  warnings.warn('Stats were not generated because "stats" parameter was set to "False".')
    ...
    def _main_result(self):
        ...
        if len(self.__difpy_obj._group_to_id_dictionary) == 0:
            # union
            with Pool() as pool:       
    ...
    def _search_helper(self):
        ...
        lower_quality = []
        if self.__difpy_obj._stats['process']['build']['parameters']['in_folder']:
            # separate folders
            # calculate the number of similar images and duplicates found, and compare image quality
...
class _initialize:
    '''
    A class used by difPy to initialize input files.
    '''
    def __init__(self, directory, recursive, in_folder, limit_extensions, px_size, show_progress):
    ...
    def _stats(self, **kwargs):
        ...
        stats.update({'directory' : self.directory})
        stats.update({'process' : {'build': {}}})   
        stats['process']['build'].update({'duration' : {'start': self.start_time.isoformat(),
                                                        'end' : self.end_time.isoformat(),
                                                        'seconds_elapsed' : seconds_elapsed
                                                       }})
        stats['process']['build'].update({'parameters': {'recursive' : self.recursive,
                                                         'in_folder' : self.in_folder,
                                                         'limit_extensions' : self.limit_extensions,
    ...
    def _get_files(self):
        ...
        if self.in_folder:
            ...
            for dir in self.directory:
                directories += glob(str(dir) + '/**/', recursive=self.recursive)
            for dir in directories:
                files = glob(str(dir) + '/*', recursive=self.recursive)
    ...
    def _validate_files(self, directory): # list_all_files in difPy v3
    ...
    def _build_image_dictionaries(self, valid_files):
        ...
        count = 0
        if self.in_folder:
            total_count = sum( [ len(i) for i in valid_files])
        else:
    ...
    def _generate_tensor(self, num, file):
        try:
            ...
            if img.getbands() != ('R', 'G', 'B'):
                img = img.convert('RGB')
            img = img.resize((self.px_size, self.px_size), resample=Image.BICUBIC)
            img = np.asarray(img)
            return (num, img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-D', '--directory', type=str, nargs='+', help='Directory to search for images.', required=False, default=os.getcwd())
    ...
    parser.add_argument('-i', '--in_folder', type=lambda x: bool(strtobool(x)), help='Scan subfolders for duplicate images.', required=False, choices=[True, False], default=False)    
    ...
    parser.add_argument('-l', '--logs', type=lambda x: bool(strtobool(x)), help='Enable collection of stats during the process.', required=False, choices=[True, False], default=True)
    ...
    lq_file = f'difPy_{timestamp}_lower_quality.csv'
    ...
    if args.move_to != None:
        ...
        search.lower_quality = se.move_to(se, args.move_to).lower_quality

    # output 'search.lower_quality' to file
    with open(lq_file, 'w') as file:
        wr = csv.writer(file, quoting=csv.QUOTE_ALL)
        wr.writerow(se.lower_quality)
